	...
	if($video['type'] == "file") { //JWPLAYER
		...
		if($video['type'] == "youtube") { //유튜브
			...
			*/
			...
			$autoplay = ($autoplayvideo || $video['auto']) ? 'true' : 'false';
			$show = '<iframe src="https://tv.naver.com/embed/'.$video['vid'].'?autoPlay='.$autoplay.'" frameborder="no" scrolling="no" marginwidth="0" marginheight="0" WIDTH="'.$video['width'].'" HEIGHT="'.$video['height'].'" allow="autoplay"'.$fullscreen.'></iframe>';
...
function apms_wr_thumbnail($bo_table, $write, $thumb_width, $thumb_height, $is_create=false, $is_crop=false, $crop_mode='center', $is_sharpen=true, $um_value='80/0.5/3') {
	...
	if($make_thumb) {
		...
		$result = sql_query(" select bf_file, bf_content from {$g5['board_file_table']} where bo_table = '$bo_table' and wr_id = '$wr_id' and bf_type in (1, 2, 3, 18) order by bf_no", false);
		$z = 0;
		for ($i=0; $row=sql_fetch_array($result); $i++) {
			if($row['bf_file']) {
...
function thema_member($opt=''){
	...
	$member['cart'] = (int)$member['cart'];
	$member['today'] = (int)$member['today'];
...
function apms_script($name){
	...
	if($name == 'code') {
		...
		if(!defined('APMS_BGVIDEO')) {
			...
			$youtube_key = (defined('APMS_YOUTUBE_KEY') && APMS_YOUTUBE_KEY) ? APMS_YOUTUBE_KEY : 'AIzaSyB_z8WrsKw2kIplh5kBc6xTawEDu91V5dQ';
			echo '<script>'.PHP_EOL;
...
function apms_editor_image($content, $mode='') {
	...
	for($i=0;$i < $imgs_cnt;$i++) {
		...
		$$data_path = clean_relative_paths($data_path);
		
		$is_destfile = false;
        if(preg_match('/(gif|jpe?g|bmp|png|webp)$/i', strtolower(end(explode('.', $data_path))))){
