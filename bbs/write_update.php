...
if($board['as_resize_kb'] > 0 && $board['as_resize'] > 0) {
    ...
    while ($row = sql_fetch_array($result)) {
		...
		if($thumb && $thumb != $org) {
            ...
            sql_query(" update {$g5['board_file_table']} set bf_filesize = '".(int)$tsize."', bf_width = '".(int)$timg[0]."', bf_height = '".(int)$timg[1]."', bf_type = '".(int)$timg[2]."' where bo_table = '{$bo_table}' and wr_id = '{$wr_id}' and bf_no = '{$row['bf_no']}' ", false);
		}
	}
