...
for ($i = 1; $i <= 10; $i++) {
	...
	$thumb = get_it_thumbnail($it['it_img' . $i], 600, 600);
	...
	if ($thumb) {
		...
		$thumbnails[$j] = '<a data-href="" data-ref="" class="thumb_item_image">' . $thumb . '</a>';
		$j++;
	}
}
$slider_images = array();
for ($i = 1; $i <= 10; $i++) {
	if (!$it['it_img' . $i])
		continue;
	$s_img = get_it_thumbnail($it['it_img' . $i], 600, 600);

	if ($s_img) {
		$org_url = G5_DATA_URL . '/item/' . $it['it_img' . $i];
		$img = apms_thumbnail($org_url, 600, 600, false, true);
		$s_img_url = ($img['src']) ? $img['src'] : $org_url;
		if ($j == 0) {
			$item_image = $s_img_url; // 큰이미지
			$item_image_href = G5_SHOP_URL . '/largeimage.php?it_id=' . $it['it_id'] . '&amp;ca_id=' . $ca_id . '&amp;no=' . $i; // 큰이미지 주소
		}
		$slider_images[$j] = '<a data-href="' . G5_SHOP_URL . '/largeimage.php?it_id=' . $it['it_id'] . '&amp;ca_id=' . $ca_id . '&amp;no=' . $i . '" data-ref="' . $thumb_url . '" class="thumb_item_image">' . $thumb . '<span class="sound_only"> ' . $i . '번째 이미지 새창</span></a>';
		$j++;
	}
}
...
<div class="item-head shop-details-wrap">
	<div class="row">
		<div class="col-sm-5 col-xs-4">
			...
			<div id="item_image" class="swiper details-slider">
				<div class="swiper-wrapper">
					<?php
					for ($i = 0; $i < count($thumbnails); $i++) {
						echo '<div class="swiper-slide"><img style="object-fit: fill;" src="' . $thumbnails[$i] . '</div>';
					}
					?>
				</div>
		...
		<div class="col-sm-7 col-xs-7">
			...
			<div class="shop-details-table-line"></div>
			<form name="fitem" method="post" action="<?php echo $action_url; ?>" class="form item-form" role="form" onsubmit="return fitem_submit(this);">
				...
				<table class="div-table table">
					...
					<tbody>
						...
						?>
							...
							<tr>
								<th class="detail-table-tit" scope="row">Status</th>
				...
				*/
						...
						<?php } ?>
						<?php
						$ct_send_cost_label = 'Shipping';

						if ($it['it_sc_type'] == 1)
							$sc_method = 'Direct transaction';
						else {
							if ($it['it_sc_method'] == 1)
								...
								$sc_method = '<select name="ct_send_cost" id="ct_send_cost" class="form-control input-sm">
										  ...
										  <option value="1">Delivery transaction</option>
									  </select>';
							} else
						...
						<tr>
							<th>Share this products</th>
							<td><?php include_once(G5_SNS_PATH . "/item.sns.skin.php"); ?></td>
						</tr>
